# -*- coding: utf-8 -*-
"""
Created on Fri Nov 04 11:31:28 2016
...
#    
    ...
    def test_time_course_pruning1(self):
        before_pruning=pandas.DataFrame.from_csv(self.timecourse_report_name,sep='\t',index_col=None)
        PCH=PyCoTools.pycopi.PruneCopasiHeaders(self.timecourse_report_name,
                                                   replace='false',new_path=self.timecourse_report_name2)
        after_pruning=pandas.DataFrame.from_csv(self.timecourse_report_name2,sep='\t',index_col=None)
        self.assertNotEqual( list(before_pruning.keys()),list(after_pruning.keys()))

    def test_time_course_pruning2(self):
        before_pruning=pandas.DataFrame.from_csv(self.timecourse_report_name,sep='\t',index_col=None)
        PCH=PyCoTools.pycopi.PruneCopasiHeaders(self.timecourse_report_name,
                                                   replace='false',new_path=self.timecourse_report_name2)
        after_pruning=pandas.DataFrame.from_csv(self.timecourse_report_name2,sep='\t',index_col=None)
        new_titles=[]        
        for i in before_pruning.keys():
            match= re.findall('.*\[(.*)\]',i)
            if match==[]:
                new_titles.append(i)
        ...
        before_pruning.columns=new_titles
        self.assertListEqual(list(before_pruning.keys()),list(after_pruning.keys()))
        
    def test_PE_file_pruning1(self):
        '''
        Test the pruning of PE data file
        '''
        before_pruning=pandas.DataFrame.from_csv(self.PE_report_name,sep='\t',index_col=None)
        PCH=PyCoTools.pycopi.PruneCopasiHeaders(self.PE_report_name,replace='true')
        after_pruning=pandas.DataFrame.from_csv(self.PE_report_name,sep='\t',index_col=None)
        self.assertNotEqual( list(before_pruning.keys()),list(after_pruning.keys()))


    def test_PE_file_pruning2(self):
        '''
        Test the pruning of PE data file
        '''
        before_pruning=pandas.DataFrame.from_csv(self.PE_report_name,sep='\t',index_col=None)
