...
#        self.PE.run()
        ...
        self.PL=PyCoTools.pydentify2.ProfileLikelihood(self.copasi_file,
                                                       ...
                                                       Index=0,IterationLimit=10,
                                                       Run='false')
        self.GEP=PyCoTools.pycopi.GetModelQuantities(self.copasi_file)
#        
#    def test_read_PE_data(self):
#        self.assertEqual(self.PL.PE_data.shape[0],int(self.S.kwargs.get('NumberOfSteps')))
#
#    def test_cps_copying(self):
#        '''
#        check a copasi file exists for each parameter in fit task
#        '''
#        fit_items= self.GEP.get_fit_items().keys()
#        for i in self.PL.cps_dct:
#            self.assertListEqual(sorted(self.PL.cps_dct[i].keys()),sorted(fit_items))
#
#  
#    def test_read_data_from_picklefalse_overwrite_pickle_true(self):
#        PED=PyCoTools.PEAnalysis.ParsePEData(self.PE_report_name)
#        self.assertTrue(isinstance( PED.read_data(),pandas.core.frame.DataFrame))
        
    def test_insert_parameters(self):
        print self.PL.PE_data
        key1= self.PL.cps_dct.keys()[0]
        key2= self.PL.cps_dct[key1].keys()[0]
        cps=self.PL.cps_dct[key1][key2]
        GMQ=PyCoTools.pycopi.GetModelQuantities(cps)
        print pandas.DataFrame.from_dict(GMQ.get_all_params_dict())
#        
      
#    def test_read_PE_data_none(self):
#        PL=PyCoTools.pydentify2.ProfileLikelihood(self.copasi_file)
#        self.assertEqual(PL.read_PE_data(),None)
#        
#    def test_read_PE_data_file(self):
#        PL=PyCoTools.pydentify2.ProfileLikelihood(self.copasi_file,ParameterPath=self.PE_report_name)
#        self.assertTrue(isinstance(PL.read_PE_data(),pandas.core.frame.DataFrame))
#        
#    def test_read_PE_data_folder(self):
#        PL=PyCoTools.pydentify2.ProfileLikelihood(self.copasi_file,ParameterPath=self.PE_dir)
#        self.assertTrue(isinstance(PL.read_PE_data(),pandas.core.frame.DataFrame))
#
#    def test_write_cps_files_minus1(self):
#        '''
#        write cps fiels when mode is set to -1 (i.e. use current parameters)
#        '''
#        PL=PyCoTools.pydentify2.ProfileLikelihood(self.copasi_file)
#        if PL.kwargs.get('Index')==-1:
#            dire= os.path.join(os.path.dirname(self.copasi_file),'ProfileLikelihood')
#            dire=os.path.join(dire,'-1')
#            test_cps_files= [os.path.join(dire,i+'.cps') for i in PL.GMQ.get_fit_items().keys()]
#            [self.assertTrue(os.path.isfile(i)) for i in test_cps_files]
#
#    def test_write_cps_files_int(self):
#        PL=PyCoTools.pydentify2.ProfileLikelihood(self.copasi_file,Index=0,ParameterPath=self.PE_dir)
#        if isinstance(PL.kwargs.get('Index'),int):
#            dire= os.path.join(os.path.dirname(self.copasi_file),'ProfileLikelihood\{}'.format(PL.kwargs.get('Index')))
#            test_cps_files= [os.path.join(dire,i+'.cps') for i in PL.GMQ.get_fit_items().keys()]
#            [self.assertTrue(os.path.isfile(i)) for i in test_cps_files]
#
#    def test_write_cps_files_list(self):
#        PL=PyCoTools.pydentify2.ProfileLikelihood(self.copasi_file,Index=[0],ParameterPath=self.PE_dir)
#        if isinstance(PL.kwargs.get('Index'),list):
#            for i in PL.kwargs.get('Index'):
#                dire= os.path.join(os.path.dirname(self.copasi_file),'ProfileLikelihood\{}'.format(i))
#                test_cps_files= [os.path.join(dire,j+'.cps') for j in PL.GMQ.get_fit_items().keys()]
#                [self.assertTrue(os.path.isfile(j)) for j in test_cps_files]
#
#    def test_insert_parameters_int_index(self):
#        '''
#        insert parameters when index is type int
#        '''
        
#        PL=PyCoTools.pydentify2.ProfileLikelihood(self.copasi_file,Index=0,ParameterPath=self.PE_dir)
#        print PL.copasiML_dct
#          
#    def test_insert_parametes(self):
#        print os.getcwd()
#        PL=PyCoTools.pydentify2.ProfileLikelihood(self.copasi_file,Index=0,ParameterPath=self.PE_dir)         
#        print os.getcwd()        
#        cps_dct=PL.insert_parameters()
#        print cps_dct
#        
#    def test_write_cps_files_int(self):
#        PL=PyCoTools.pydentify2.ProfileLikelihood(self.copasi_file,Index=[0],ParameterPath=self.PE_dir)
...
#                os.mkdir(dire)
#            os.chdir(dire)
#            for i in self.copasiML_dct:
#                for j in self.copasiML_dct[i]:
#                    f_name=os.path.join(dire,j+'.cps')
#                    f_name_dct[-1][j]=f_name
#                    self.CParser.write_copasi_file(f_name,self.copasiML_dct[i][j])
#            return f_name_dct
#
#
#
#    def test_get_copasiML_dict_current(self):
#        '''
#        A test to see if the model structure works for model parameters.
#        For this, no additional arguments are required.
#        '''
#        PL=PyCoTools.pydentify2.ProfileLikelihood(self.copasi_file)
#        self.assertTrue(isinstance(PL.insert_parameters(),dict))
#
#    def test_get_copasiML_dict_file_int_index(self):
#        '''
#        A test to see if the model structure works for model parameters.
#        For this, no additional arguments are required.
#        '''
#        PL=PyCoTools.pydentify2.ProfileLikelihood(self.copasi_file,Index=0,ParameterPath=self.PE_file)
#        self.assertTrue(isinstance(PL.insert_parameters(),dict))        
#
#    def test_get_copasiML_dict_file_list_index(self):
#        '''
#        A test to see if the model structure works for model parameters.
#        For this, no additional arguments are required.
#        '''
#        PL=PyCoTools.pydentify2.ProfileLikelihood(self.copasi_file,Index=[0,1,2],ParameterPath=self.PE_dir)
#        self.assertTrue(isinstance(PL.insert_parameters(),dict)) 
#        
#    def test_insert_parameters_no_insert(self):
#        PL=PyCoTools.pydentify2.ProfileLikelihood(self.copasi_file)
#        copasi_dct= PL.insert_parameters()
#        self.assertTrue( copasi_dct.keys()[0]==-1)
#
#    def test_insert_parameters_insert_int_index(self):
#        PL=PyCoTools.pydentify2.ProfileLikelihood(self.copasi_file,Index=0,ParameterPath=self.PE_dir)
#        copasi_dct=PL.insert_parameters()
#        parameters=  PL.PE_data.iloc[0]
##        print PL.GMQ.get_global_quantities()['Kcd']
#        testML= copasi_dct[0].keys()[0]
#        self.assertTrue( copasi_dct.keys()[0]==-1)        
...
#        import time
#        time.sleep(5)
#        d=os.path.join(os.path.dirname(self.copasi_file),'ProfileLikelihood')
#        if os.path.isdir(d):
#            shutil.rmtree(d)
...
#        PED=ParsePEData(self.PE_dir,FromPickle=True,OverwritePickle=False)
#        self.assertTrue(isinstance( PED.read_data(),pandas.core.frame.DataFrame))
#
#    def test_read_data_from_pickle_true_overwrite_pickle_true(self):
#        PED=ParsePEData(self.PE_dir,FromPickle=True,OverwritePickle=True)
#        self.assertTrue(isinstance( PED.read_data(),pandas.core.frame.DataFrame))
#        
#    def test_read_data_from_pickle_false_overwrite_pickle_true(self):
#        PED=ParsePEData(self.PE_dir,FromPickle=False,OverwritePickle=True)
#        self.assertTrue(isinstance( PED.read_data(),pandas.core.frame.DataFrame))
#
#    def test_read_data_from_pickle_false_overwrite_pickle_false(self):
#        PED=ParsePEData(self.PE_dir,FromPickle=False,OverwritePickle=False)
#        self.assertTrue(isinstance( PED.read_data(),pandas.core.frame.DataFrame))
#                      
##    def test_pickle_data_frompickle_false(self):
##        PED=ParsePEData(self.PE_dir,FromPickle=False)
