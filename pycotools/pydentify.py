...
"""
 ...
 $Date: 12-09-2016
 Time:  14:50
  This file uses the tasks module to set up and run a profile likleihool
...
#
    ...
    def read_PE_data(self):
        ...
        if self.mode=='current':
            ...
            return viz.ParsePEData(self.kwargs.get('parameter_path')).data
    ...
    def copy_copasi_files_and_insert_parameters(self,IA_dir=None):
        ...
        if self.kwargs.get('index')==-1:
            ...
            IP=tasks.InsertParameters(self.copasi_file,
    ...
    def setup_PE_task(self):
        ...
        method_element=tasks.etree.Element('Method',attrib=method_params)
        tasks.etree.SubElement(method_element,'Parameter',attrib=iteration_limit)
        tasks.etree.SubElement(method_element,'Parameter',attrib=tolerance)
        tasks.etree.SubElement(method_element,'Parameter',attrib=rho)
            
            
        query="//*[@name='FitItem']" 
        for i in self.cps_dct:
            for j in self.cps_dct[i]:
                C= tasks.CopasiMLParser(self.cps_dct[i][j])
                childML=C.copasiML
    ...
    def setup_scan(self):
        for i in self.cps_dct:
            for j in self.cps_dct[i]:
                GMQ_child=tasks.GetModelQuantities(self.cps_dct[i][j])
    ...
    def multi_run(self):
        ...
        def run(f):
            ...
            tasks.Run(f, task = 'scan', mode='multiprocess').run()
...
class Plot2():
    ...
    **kwargs:
        ...
        use_pickle:
            Data read by viz.ParsePEData are automatically pickled
...
#        return sorted(self.data[first_index].keys())
    ...
    def get_RSS(self):
        ...
        if self.kwargs.get('index')==-1:
            ...
            PED= viz.ParsePEData(self.kwargs.get('parameter_path'))
            if isinstance(self.kwargs.get('index'),int):
