...
class TestParameterEstimationVizWithMultipleIndependentVars(unittest.TestCase):
    """
    ...
    def setUp(self):
        ...
        for i in [10, 20, 30, 40]:
            ...
            pycotools.model.InsertParameters(self.mod, parameter_dict={'Signal': i}, inplace=True)

            ## create unique report name
            report_name = os.path.join(self.working_directory, 'timecourse_signal_{}.txt'.format(i))

            ## simulate time course. Turn off recording global quantities
            self.TC_dict[i] = pycotools.tasks.TimeCourse(
                self.mod, end=10, intervals=10, step_size=1, report_name=report_name,
                global_quantities=[], y=['A', 'B']
            )

            ## format the time course data
            df = pycotools.misc.format_timecourse_data(self.TC_dict[i].report_name)

            ## create independent variable column
            df['Signal_indep'] = [i] * df.shape[0]

            ## write data to file
            df.to_csv(self.TC_dict[i].report_name, sep='\t', index=False)
    ...
    def test_that_12_graphs_are_Saved_to_file(self):
        PE2 = pycotools.tasks.ParameterEstimation(self.mod, [i.report_name for i in self.TC_dict.values()],
        ...
        PE2.run()
        PPE = pycotools.viz.PlotParameterEstimation(PE2, savefig=True)

        print PPE.results_directory

        dirs = glob.glob(os.path.join(PPE.results_directory, 'timecourse_signal*'))

        count = 0
        for i in dirs:
            pngs = glob.glob(os.path.join(i, '*.png'))
            count = count + len(pngs)
