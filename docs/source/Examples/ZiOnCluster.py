...
from pycotools.Examples.zi_model_varients import Models
import pandas
...
directory = r'/sharedlustre/users/b3053674/2017/10_Oct/Smad7ZiModels2'
...
PE = tasks.MultiModelFit(
    ...
    method='genetic_algorithm', population_size=300, number_of_generations=500,
...
if PLOT:
    ...
    for m in PE:
        observables = ['Smad7', 'Smad7Obs', 'Ski',
                       'Smad7_mRNA', 'Ski_mRNA',
                       'SkiObs', 'Smad7mRNAObs']
        TCM = viz.PlotTimeCourseEnsemble(m, savefig=False, show=True, truncate_mode='percent', theta=10)
        # viz.RssVsIterations(m, savefig=True, log10=True)
        # viz.Boxplots(m, savefig=True, log10=True)
        # viz.Histograms(m, savefig=True, log10=True)
        # viz.Pca(m, by='iterations', savefig=True)
        # viz.Pca(m, by='parameters', savefig=True, legend_position=(10, 10, 1.5))



    # MS = viz.ModelSelection(PE, savefig=True, dpi=1000)
    # MS.to_pickle()
